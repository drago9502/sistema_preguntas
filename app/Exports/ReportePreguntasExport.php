...
class ReportePreguntasExport implements FromView, ShouldAutoSize
{
    ...
    public function view(): View
    {
        ...
        $presencialese = Asistente::where('id_evento', $evento->id)->where('status', 1)->where('modalidad', 'Presencial')->count();
        $virtualese = Asistente::where('id_evento', $evento->id)->where('status', 1)->where('modalidad', 'Virtual')->count();
        ...
        return view('admin.preguntas.reporte_excel', ['evento' => $evento, 'preguntas' => $preguntas, 'registrados' => $registrados,'virtuales' => $virtualese, 'presenciales' => $presencialese]);
    }
}
